...
def make_collate_fn(dataset: TrainingDataset, quantizer, batch_split_size=None):
    ...
    def collate_fn(data_list):
        ...
        # list_of_tensors = [torch.tensor([[1, 2, 3], [3, 4, 5]]), torch.tensor([[5, 6, 7], [7, 8, 9]])]
        # list_of_tensors = torch.stack(list_of_tensors)
        # print('###########################################################################################')
        # print('#####################')
        # print('data_list_size:', len(data_list))
        clouds = [e[0] for e in data_list]
        # print('clouds_size:', len(clouds))
        labels = [e[1] for e in data_list]

        batch_graph = torch.stack(clouds)
        # print('batch_graph_size:', batch_graph.size())
        batch_graph = batch_graph[:, -12:, :]
        # print('#####################')
        # print('###########################################################################################')
        ...
        return batch, batch_graph, positives_mask, negatives_mask

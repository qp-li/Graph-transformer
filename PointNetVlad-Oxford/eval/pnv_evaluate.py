...
def evaluate(model, device, params: TrainingParams, log: bool = False, show_progress: bool = False):
    ...
    for database_file, query_file in zip(eval_database_files, eval_query_files):
        ...
        assert location_name == temp, 'Database location: {} does not match query location: {}'.format(database_file,
                                                                                                       query_file)
        ##读取原始数据，已经保存在Pickle中，可以在dataset.generate中，重新把bin数据换成我们的json数据
...
def evaluate_dataset(model, device, params: TrainingParams, database_sets, query_sets, log: bool = False,
    ...
    # get_latent_vectors  相当于编码器，把每个原始数据变成了256维的描述子，保存到了database_embeddings中
    # 修改此处的get_latent_vectors，换成SGPR的encoder
...
def get_latent_vectors(model, set, device, params: TrainingParams):
    ...
    for i, elem_ndx in enumerate(set):
        ...
        pc = pc_loader(pc_file_path)   #读取所有点云 (空间+语义) N*3,没有Batch
        pc = torch.tensor(pc)
...
def compute_embedding(model, pc, device, params: TrainingParams):
    Point_umber = 4096
    graph_umber = 12
    coords, _ = params.model_params.quantizer(pc[:Point_umber, :])
    with torch.no_grad():
        bcoords = ME.utils.batched_coordinates([coords])
        feats = torch.ones((bcoords.shape[0], 1), dtype=torch.float32)
        batch = {'coords': bcoords.to(device), 'features': feats.to(device)}
        batch_graph = pc[(-graph_umber):, :]
        # Compute global descriptor
        y = model(batch, batch_graph)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, required=False, default='../config/config_refined.txt', help='Path to configuration file')
    parser.add_argument('--model_config', type=str, required=False, default='../models/minkloc.txt', help='Path to the model-specific configuration file')
    parser.add_argument('--weights', type=str, required=False, default='../weights/Oxford_bast_model.pth', help='Trained model weights')
